 ...
 *
 * @credits
 * Added Toolbar by Stephanus Ba.Sa (wiefung@sbskokmputer.web.id)
 *
 * @license
...
var SyntaxHighlighter = function () {
    ...
    var e = {
        ...
        config: {
			...
			clipboardSwf: null,
			toolbarItemWidth: 12,
			toolbarItemHeight: 12,
            bloggerMode: false,
            stripBrs: false,
            tagName: "pre",
            strings: {
				expandGutter: "expand gutter",
                ...
                aboutDialog: '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><title>About SyntaxHighlighter</title></head><body style="font-family:Geneva,Arial,Helvetica,sans-serif;background-color:#fff;color:#000;font-size:1em;text-align:center;"><div style="text-align:center;margin-top:1em;"><div style="font-size:xx-large;">SyntaxHighlighter</div><div style="font-size:.75em;margin-bottom:2em;"><div>version 3.0.83 (July 02 2010)</div><div><a href="http://alexgorbatchev.com/SyntaxHighlighter" target="_blank" style="color:#005896">http://alexgorbatchev.com/SyntaxHighlighter</a></div><div>JavaScript code syntax highlighter.</div><div>Copyright 2004-2010 Alex Gorbatchev.</div><br /><div>Credit of Toolbar Script by</div><div>Stephanus Ba.Sa (wiefung@sbskomputer.web.id)</div><div><a href="http://sbskomputer.web.id" target="_blank" style="color:#005896">http://sbskomputer.web.id</a></div></div><div>If you like this script, please <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=2930402" style="color:#005896">donate</a> to <br/>keep development active!</div></div></body></html>'
            }
        },
        vars: {
        ...
        utils: {
			...
			unindent: function(str) {
				var a = e.utils.fixInputString(str).split("\n"),
					b = new Array(),
					c = /^\s*/,
					min = 1000;
				for (var i = 0; i < a.length && min > 0; i++) {
					var d = a[i];
					if (e.utils.trim(d).length == 0) {
						continue
					}
					var f = c.exec(d);
					if (f == null) {
        ...
        toolbar: {
            ...
            getButtonHtml: function (a, b, c) {
                var w = e.config.toolbarItemWidth,
					h = e.config.toolbarItemHeight;
                return '<a href="#" class="toolbar_item command_' + b + " " + b + '" style="width:' + w + 'px!important;height:' + h + 'px!important" title="' + c + '"><span>' + c + "</span></a>"
            },
            handler: function (a) {
                var b = a.target,
                    c = b.className || "";
            ...
            items: {
                list: ["expandSource", "expandGutter", "copyToClipboard", "print", "viewSource", "help"],
                expandSource: {
                    getHtml: function (a) {
                ...
                expandGutter: {
					getHtml: function (a) {
                        var b = e.toolbar.getButtonHtml(a, "expandGutter", e.config.strings.expandGutter);
                        b = b.replace(" expandGutter", " expandGutter opened");
                        return b
                    },
                    execute: function (a) {
						e.defaults.gutter = false;
                        a = document.getElementById(t(a.id));
                        var b = p(a, ".toolbar", false),
							c = p(b, ".expandGutter", false);
						if (!(!c)) {
							if(c.className.indexOf("opened") > 0) c.className = c.className.replace(" opened", "");
							if(a.className.indexOf("nogutter") > 0) {
								c.className += " opened";
								c.className = c.className.replace(" closed", "");
								a.className = a.className.replace(" nogutter", "")
							} else {
								c.className += " closed";
								c.className = c.className.replace(" opened", "");
								a.className += " nogutter"
							}
						}
                    }
                },
                copyToClipboard: {
					getHtml: function (a) {
                        return e.toolbar.getButtonHtml(a, "copyToClipboard", e.config.strings.copyToClipboard)
                    },
                    execute: function (a) {
							a = document.getElementById(t(a.id));
							var b = p(a, ".syntaxhighlighter", false);
							a = p(a, ".container", false);
							var c = document.createElement("textarea");
							if (!(!a || !b || p(a, "textarea"))) {
								B(b.id);
								r(b, "source");
								...
								window.alert(e.config.strings.copyToClipboardConfirmation);
                ...
                print: {
					getHtml: function (a) {
                        return e.toolbar.getButtonHtml(a, "print", e.config.strings.print)
                    },
                    execute: function (c) {
                        var a = "location=0, resizable=1, menubar=0, scrollbars=1";
                        var w = (screen.width - 200),
							h = (screen.height - 200);
                        a += ", left=" + (screen.width - w) / 2 + ", top=" + (screen.height - h) / 2 + ", width=" + w + ", height=" + h;
                        a = a.replace(/^,/, "");
                        a = window.open("", "_blank", a);
                        a.focus();
                        var b = a.document;
                        b.write("<pre>" + e.code + "</pre>");
                        b.close();
                        a.focus();
                        a.print()
                    }
                ...
                viewSource: {
                    getHtml: function (a) {
                        return e.toolbar.getButtonHtml(a, "viewSource", e.config.strings.viewSource)
                    },
                    execute: function (c) {
    ...
    e.Highlighter.prototype = {
        ...
        getHtml: function (a) {
            ...
            if (this.getParam("wrap-lines") == false) c.push("no-wrap");
            ...
            return b = '<div id="' + t(this.id) + '" class="' + c.join(" ") + '">' + (this.getParam("toolbar") ? e.toolbar.getHtml(this) : "") + '<div class="syntax-content"><table border="0" cellpadding="0" cellspacing="0">' + this.getTitleHtml(this.getParam("title")) + "<tbody><tr>" + (gutter ? '<td class="gutter">' + this.getLineNumbersHtml(a) + "</td>" : "") + '<td class="code"><div class="container">' + b + "</ioldiv></td></tr></tbody></table></div></div>"
        },
        getDiv: function (a) {
            if (a === null) a = "";
            this.code = a;
